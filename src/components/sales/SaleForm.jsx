...
const SaleForm = ({ sale = null, onSubmit, onCancel }) => {
  ...
  const [saleType, setSaleType] = useState(sale?.type || 'proforma');
  const [formData, setFormData] = useState({
    customer_id: parseInt(sale?.customer_id) || '',
    warehouse_id: parseInt(sale?.warehouse_id) || '',
    items: sale?.items || [{
      product_id: '',
      productName: '',
      productTypeId: '',
  ...
  const loadCustomers = async () => {
    try {
      ...
      //console.log(proData)
      setProducts(proData.data.map(pro => ({
        value: String(pro.id),
        label: `${pro.name} - (${GetStock(pro.inventory)})`,
        price: pro.max_price,
        ...
        <div className="space-y-3">
          {formData.items.map((item, index) => (
            <div key={index} className="border rounded-lg p-4 bg-gray-50">
              <div className="grid md:grid-cols-12 grid-cols-2 gap-3">
...
// const SaleForm = ({ sale = null, onSubmit, onCancel }) => {
//   const [saleType, setSaleType] = useState(sale?.type || 'invoice');
//   const [formData, setFormData] = useState({
...
//   const products = [
//     { value: '1', label: 'iPhone 14 Pro 256GB', price: 125900 },
//     { value: '2', label: 'Samsung S23 128GB', price: 74999 }
//   ];
